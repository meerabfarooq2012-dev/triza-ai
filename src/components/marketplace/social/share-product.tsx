...
export function ShareProduct({
  ...
  const handleWhatsApp = () => {
    const text = `Check out "${productName}" on Marketo! ${productUrl}`
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank')
    trackShare('whatsapp')
  }

  const handleTwitter = () => {
    const text = `Check out "${productName}" on Marketo!`
    window.open(
  ...
  const shareButtons = [
    {
      ...
      color: copied ? 'text-emerald-600' : 'text-gray-600',
      bg: copied ? 'bg-emerald-50' : 'bg-gray-50',
    },
    {
      label: 'WhatsApp',
      icon: MessageCircle,
